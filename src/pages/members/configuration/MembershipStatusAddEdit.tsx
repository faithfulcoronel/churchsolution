...
function MembershipStatusAddEdit() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const isEditMode = !!id;

  const { useQuery, useCreate, useUpdate } = useMembershipStatusRepository();

  const [formData, setFormData] = useState<Partial<MembershipStatus>>({
    code: '',
    name: '',
    description: '',
    is_active: true,
  ...
  const handleInputChange = (field: keyof MembershipStatus, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode) {
        ...
        navigate(`/members/configuration/membership-status/${id}`);
      } else {
        const result = await createMutation.mutateAsync({ data: formData });
        navigate(`/members/configuration/membership-status/${result.id}`);
      }
    } catch (err) {
  ...
  return (
    <div className="w-full mx-auto px-4 sm:px-6 lg:px-8">
      <div className="mb-6">
        <BackButton fallbackPath="/members/configuration/membership-status" label="Back" />
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            <div className="flex items-center">
              ...
              <h3 className="text-lg font-medium">
                {isEditMode ? 'Edit Membership Status' : 'Add New Membership Status'}
              </h3>
            </div>
          </CardHeader>
          ...
          <CardFooter className="flex justify-end space-x-3">
            <Button type="button" variant="outline" onClick={() => navigate('/members/configuration/membership-status')}>
              Cancel
            </Button>
...
export default MembershipStatusAddEdit;
